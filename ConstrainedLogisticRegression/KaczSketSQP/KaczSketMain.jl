include("KaczSketSQP.jl")

struct KaczSketResult
    XStep::Array
    LamStep::Array
    KKTStep::Array
    alpha_Step::Array
...
function KaczSketMain(Aug, Prob)
    Verbose = Aug.verbose
    ...
    rho = Aug.rho
    # Final result object
    KaczSketR = Array{KaczSketResult}(undef,length(Prob))
    ## Go over all Problems
    for Idprob = 1:length(Prob)
        # load problem
        println(Prob[Idprob]) #Prob[Idprob]
        ...
        rep = 1
        while rep <= TotalRep
            println("KaczSketSQP","-",Idprob,"-",rep)
            X,Lam,KKT,Alpha,grad_eval,objcon_eval,Time,IdCon,IdSing = KaczSketSQP(feature,label,con_A,con_b,Max_Iter,EPS_Res,eta1,eta2,delta,xi_B,beta,nu,rho)
            if IdSing == 1
                break
            elseif IdCon == 0
        ...
        KaczSketR[Idprob] = KaczSketResult(XStep,LamStep,KKTStep,alpha_Step,Grad_eval,Objcon_eval,TimeStep)
    end
    return KaczSketR
end
