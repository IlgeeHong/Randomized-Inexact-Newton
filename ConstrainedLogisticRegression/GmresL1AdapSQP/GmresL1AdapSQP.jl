...
function GmresL1AdapSQP(feature,label,con_A,con_b,Max_Iter,EPS_Res,mu,kappa,kappa1,epsilon,tau,eta,xi_B)
    # data information
    N = size(feature)[1]
    nx = size(feature)[2]
    nlam = 11
    ...
    while KKT[end]>EPS_Res && k<Max_Iter
        ...
        if xi_G < 1e-4
            ...
            if nlam<nx && eigmin(Hermitian(Q_k[:,nlam+1:end]'nab_x2L_k*Q_k[:,nlam+1:end],:L)) < 1e-4
            ...
            while true
                ...
                t = 0
                while norm(r,1)>Quant2 && t < 100000
                ...
                elseif model_reduc < Quant4
                    mu *= 1.4
                    # max((nabf_k'NewDir_t[1:nx]+max((1/2)*NewDir_t[1:nx]'*B_k*NewDir_t[1:nx],0))[1]/(1-tau)/(norm(c_k,1)-norm(G_k*NewDir_t[1:nx]+c_k,1))[1], mu) + 1e-4
                    kappa *= (1/(1.4)^2)
                else
                    NewDir = NewDir_t
                    break
