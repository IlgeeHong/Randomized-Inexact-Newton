...
function GaussSketSQP(feature,label,con_A,con_b,Max_Iter,EPS_Res,eta1,eta2,delta,xi_B,beta,nu,rho)
    # data information
    N = size(feature)[1]
    nx = size(feature)[2]
    nlam = 11
        ...
        if xi_G < 1e-2
            ...
            while true
                ...
                while norm(r) > Quant2 && t < 1000000
                    ...
                    S = rand(Normal(0,1),(nlam+nx,)) #rand(MvNormal(zeros(nx+nlam),Diagonal(ones(nx+nlam))))
