...
function GmresMain(Aug, Prob)
    ...
    rho = Aug.rho
    # Final result object
    GmresR = Array{GmresResult}(undef,length(Prob))
    ...
    for Idprob = 1:length(Prob)
        ...
        rep = 1
        while rep <= TotalRep
            println("GmresSQP","-",Idprob,"-",rep)
            X,Lam,KKT,Alpha,grad_eval,objcon_eval,Time,IdCon,IdSing = GmresSQP(feature,label,con_A,con_b,Max_Iter,EPS_Res,eta1,eta2,delta,xi_B,beta,nu,rho)
            if IdSing == 1
                break
            elseif IdCon == 0
        ...
        GmresR[Idprob] = GmresResult(XStep,LamStep,KKTStep,alpha_Step,Grad_eval,Objcon_eval,TimeStep)
    end
    return GmresR
end
