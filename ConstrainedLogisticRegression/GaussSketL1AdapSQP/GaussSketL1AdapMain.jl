...
function GaussSketL1AdapMain(L1Adap, Prob)
    ...
    for Idprob = 1:length(Prob)
        ...
        while rep <= TotalRep
            ...
            X,Lam,KKT,Alpha,grad_eval,objcon_eval,Time,IdCon,IdSing = GaussSketL1AdapSQP(feature,label,con_A,con_b,Max_Iter,EPS_Res,mu,kappa,kappa1,epsilon,tau,eta,xi_B)
            if IdSing == 1
                println("Singular")
                break
            elseif IdCon == 0
