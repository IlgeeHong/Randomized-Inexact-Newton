include("KaczSketL1AdapSQP.jl")

struct KaczSketL1AdapResult
    XStep::Array
    LamStep::Array
    KKTStep::Array
    alpha_Step::Array
...
function KaczSketL1AdapMain(L1Adap, Prob)
    ...
    xi_B = L1Adap.xi_B
    # Final result object
    KaczSketL1AdapR = Array{KaczSketL1AdapResult}(undef,length(Prob))
    ...
    for Idprob = 1:length(Prob)
        ...
        rep = 1
        while rep <= TotalRep
            println("KaczSketL1AdapSQP","-",Idprob,"-",rep)
            X,Lam,KKT,Alpha,grad_eval,objcon_eval,Time,IdCon,IdSing = KaczSketL1AdapSQP(feature,label,con_A,con_b,Max_Iter,EPS_Res,mu,kappa,kappa1,epsilon,tau,eta,xi_B)
            if IdSing == 1
                break
            elseif IdCon == 0
        ...
        KaczSketL1AdapR[Idprob] = KaczSketL1AdapResult(XStep,LamStep,KKTStep,alpha_Step,Grad_eval,Objcon_eval,TimeStep)
    end
    return KaczSketL1AdapR
end
